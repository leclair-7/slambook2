...
// 3D rotation matrix directly using Matrix3d or Matrix3f
  ...
  cout.precision(3);
  cout << "rotation matrix =\n" << rotation_vector.matrix() << endl;   //matrix() call converts to matrix
  ...
  cout << "quaternion from rotation vector = " << q.coeffs().transpose()
       << endl;  // Note that the order of coeffs is (x, y, z, w), w is the real part, the first
  // three are the imaginary part
